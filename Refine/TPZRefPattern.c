...
/**calcula transformacoes entre lado de filho e lado de pai*/
  ...
  for(isub=0;isub<nsubs;isub++){
    ...
    for(side=0;side<nsides;side++){/**cantos + arestas + faces, o interior não é compartilhado*/
      son->CenterPoint(masscent,side);/**percorre todos os lados do elemento filho*/
...
int TPZRefPattern::IsFatherNeighbour(TPZGeoElSide fathside,TPZGeoEl *son){
  /**si elementos filho e pai tem um lado em comum eles compartilham a vizinhança*/
  ...
  return 0;//**si não é vizinho então nenhum sub-elemento compartilha esse lado*/
}
...
TPZGeoEl *TPZRefPattern::Element(int iel){
  ...
  int nel = NSubElements()+1;/**filhos mas o pai*/
  if(iel<0 || iel>nel){
...
void TPZRefPattern::TransformationTest(){
  ...
  for(isub=0;isub<nsubs;isub++){    
    ...
    for(sd=0;sd<nsides;sd++){
      ...
      TPZIntPoints *rule = subel->CreateIntegrationRule(sd,5);
      ...
      for(ip=0;ip<rule->NPoints();ip++){
        ...
        TPZGeoElSide(subel,sd).SideTransform2(TPZGeoElSide(father,fatside),sdtosd);
