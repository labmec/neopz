...
TPZSkylMatrix::Decompose_Cholesky(std::list<int> &singular)
{
  ...
  if (  fDecomposed )  TPZMatrix::Error(__PRETTY_FUNCTION__, "Decompose_Cholesky <Matrix already Decomposed>" );
	...
	sout << "DecomposeCholesky neq = " << dimension << " average bandwidth = " << avsize << std::endl;
	std::cout << sout.str();

  singular.clear();
	REAL smallcompare = 4.e-6;
#ifdef DEBUG 
	REAL smallpivot = fElem[0][0];
	REAL largepivot = smallpivot;
#endif
  REAL pivot;
/*  if(Dim() > 100) {
...
/*    if(!(k%100) && Dim() > 100) {
    ...
    pivot = fElem[k][0] - sum;
	  ...
	  if (pivot < smallpivot) {
		  smallpivot = pivot;
	  } else if (pivot > largepivot) {
		  largepivot = pivot;
	  }
#endif
    if ( pivot < smallcompare ) {
...
#endif
  if(Rows() && (GetVal(Rows()-1,Rows()-1)) < smallcompare)
  {
    singular.push_back(Rows()-1);
    PutVal(Rows()-1,Rows()-1,1.);
  }
#ifdef DEBUG
	std::cout << "SmallPivot = " << smallpivot << std::endl;
	std::cout << "LargePivot = " << largepivot << std::endl;
	std::cout << "Number singular  = " << singular.size() << std::endl;
#endif
...
int TPZSkylMatrix::Subst_Forward( TPZFMatrix *B ) const
{
	...
	// REAL t = SubstForward.ReturnTimeDouble();
	//std::cout << "Subst_Forward: " << t << " R " << B->Rows() << " C " << B->Cols() << " Flopcount " << flopcount << " MFlop " << (flopcount/t)/1.e6 << std::endl;
...
TPZSkylMatrix::Subst_Backward( TPZFMatrix *B ) const
{
	...
	// REAL t = SubstBackward.ReturnTimeDouble();
	// std::cout << "Subst_Backward: " << t<< " R " << B->Rows() << " C " << B->Cols() << " Flopcount " << flopcount << " MFlop " << (flopcount/t)/1.e6 << std::endl;
	return( 1 );
}
