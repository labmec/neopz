...
void TPZDohrSubstructCondense::SolveSystemPhi() {
	...
	//std::cout << __FUNCTION__ << ' ' << (void *) this << " Time for U1 " << u1.ReturnTimeDouble() << std::endl;
	//TPZfTime uglob;
	fMatRedComplete->UGlobal(fKCi,rhs);
	//std::cout <<  __FUNCTION__ << ' ' << (void *) this << " Time for UGlobal " << uglob.ReturnTimeDouble() << std::endl;
	for(i=0; i<ncoarse; i++) 
	{
		fKCi(i,i) += 10.;
		for(j=0; j<ncoarse; j++)
	...
	//std::cout << __FUNCTION__ << ' ' << (void *) this << " time spent " << solvesistem.ReturnTimeDouble() << std::endl;
//	fPhiC *= -1.;
}
