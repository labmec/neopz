...
TPZDohrStructMatrix::TPZDohrStructMatrix(TPZAutoPointer<TPZCompMesh> cmesh, int numthreads_assemble, int numthreads_decompose) : 
		TPZStructMatrix(cmesh.operator->()), fDohrAssembly(0),
		fDohrPrecond(0), fMesh(cmesh)
{
	fNumThreads = numthreads_assemble;
...
TPZMatrix * TPZDohrStructMatrix::CreateAssemble(TPZFMatrix &rhs, TPZAutoPointer<TPZGuiInterface> guiInterface)
{
	...
	std::cout << "Time to assembly: " << tempo.ft5dohrassembly << std::endl;
...
#warning Tem que montar o rhs global!!
	tempo.ft51dohrdecompose = timerfordecompose.ReturnTimeDouble(); // end of timer
	std::cout << "Time to decompose: " << tempo.ft51dohrdecompose << std::endl;
...
//	subindexes.insert(index);
	...
	//	pthread_mutex_unlock(&TestThread);
		...
		matredptr->SetF(substruct->fLocalLoad);
		
		std::cout << "Norma Local Load: " << Norm(substruct->fLocalLoad) << std::endl; // apagar
		matredbig->Simetrize();
		matredptr->Simetrize();
	...
	std::cout << __FUNCTION__ << " sub index " << submesh->Index() << " total time " << totalinitialize.ReturnTimeDouble() << std::endl;
}
					  ...
					  pthread_mutex_t &TestThread)
{
		...
		TPZAutoPointer<TPZMatrix> InternalStiffness = matred->K00();
	std::list<int> singular;
		InternalStiffness->Decompose_Cholesky(singular);
	...
	// if it has a neighbour along its interior, skip
		...
		for (is=0; is<ns; is++) 
		{
			...
			for (ic=0; ic<ncomp; ic++) {
				...
				int porcaria = gelside.Element()->Dimension();
