...
void TPZMatrix<TVar>::Add(const TPZMatrix<TVar>&A,TPZMatrix<TVar>&B) const {
    ...
    for ( long r = 0; r < Rows(); r++ )
        for ( long c = 0; c < Cols(); c++ ) B.PutVal( r, c, this->GetVal(r,c)+A.GetVal(r,c) );
}
template<class TVar>
...
template <class TVar>
void TPZMatrix<TVar>::UpdateFrom(TPZAutoPointer<TPZMatrix<TVar> > mat)

{
    TPZMatrix<TVar> *matrix = mat.operator->();
    TPZMatrix<TVar> *fullmatrix = dynamic_cast<TPZMatrix<TVar> *>(matrix);
    if (!fullmatrix) {
        DebugStop();
    }
    if (this->Rows() != fullmatrix->Rows() || this->Cols() != fullmatrix->Cols()) {
        DebugStop();
    }
    this->fDecomposed = fullmatrix->fDecomposed;
    this->fDefPositive = fullmatrix->fDefPositive;
}
