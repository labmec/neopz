...
*/
	...
	PZP_THREAD_MUTEX_LOCK(submat->mutex,"MultiplySubmat()");
	...
	PZP_THREAD_MUTEX_UNLOCK(submat->mutex,"MultiplySubmat()");
};
...
//	TestZAXPY();
	...
	PZP_THREAD_MUTEX_INIT(&VectorMutex, NULL,"main()");
	int i, j, size;
...
*/
	...
	for(i=0;i<ndivisions*ndivisions;i++){
		...
		PZP_THREAD_CREATE(&mthreads[i], NULL, MultiplySubmat, data, "main()");
		counter++;
	...
	for(i=0;i<ndivisions*ndivisions;i++){
	  PZP_THREAD_JOIN(mthreads[i], NULL, "main()");
	}

	cout << "Resultado final\n";
	PZP_THREAD_MUTEX_LOCK(&VectorMutex,"main()");
	time (& endtime);
	...
	PZP_THREAD_MUTEX_UNLOCK(&VectorMutex,"main()");
}
