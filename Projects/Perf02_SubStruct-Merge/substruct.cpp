...
int main(int argc, char *argv[])
{
  ...
  TIME_SEC_BEG_LOG(perflog, timer,"Reading building mesh from ... ");
	...
	//EBORIN: o outro usa cmeshauto em vez de cmesh.
	TPZAutoPointer<TPZCompMesh> cmeshauto(cmesh);
#ifdef TIMING_ANALYSIS	
	TIME_SEC_BEG_LOG(perflog, timer,"Building the DohrmanStructMatrix");
#endif //TIMING_ANALYSIS	
	TPZDohrStructMatrix dohrstruct(cmeshauto,numthreads_nsubmesh_assemble,numthreads_decompose);
	// TPZDohrStructMatrix dohrstruct(cmesh,numthreads_nsubmesh_assemble,numthreads_decompose); // Nathan
#ifdef TIMING_ANALYSIS	
...
//		sub.SubStructure();
    ...
    //EBORIN: O outro substruct está usando: cg.SetCG(500,pre,1.e-8,0);
		//cg.SetCG(1000,pre,1.e-8,0); // Nathan
		cg.SetCG(500,pre,1.e-8,0); // Novo
		

		TPZfTime timetosolve; // init of timer
		//EBORIN: O outro substruct utiliza rhs em vez de produto
		//cg.Solve(produto,diag); // Nathan
#ifdef TIMING_ANALYSIS	
    TIME_SEC_BEG_LOG(perflog, timer,"cg.Solve");
#endif
		cg.Solve(rhs,diag); // Novo
#ifdef TIMING_ANALYSIS	
...
TPZGeoMesh *MalhaPredio()
{
	...
	//EBORIN: avoid waisting dumping the gMesh
