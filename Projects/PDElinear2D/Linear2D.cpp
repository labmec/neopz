...
//	- k.[(ddu/dxdx) + (ddu/dydy)] = f(x),   0<= x <=1    --->  Now a=1 and b=1 and  f(x)=x
...
int main()
{
	...
	TPZGeoMesh *gmesh = GeomMesh2D(h,matId,bc,x0,x1);
	gmesh->Print(infoGeoMesh);	
	
	// Creating a computational mesh and printing computational information.
	TPZCompMesh * cmesh = CompMesh2D(gmesh,p,material,bc,bcType);
