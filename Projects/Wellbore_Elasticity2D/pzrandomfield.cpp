...
TPZRandomField::TPZRandomField(TPZGeoMesh* geometricMesh, int numSquareElems,
                               int stochasticInclined, REAL direction, REAL inclination, REAL rw,
                               REAL rext, const TPZFMatrix<TVar> &M):TPZFunction<TVar>(), fPorder(-1)
{
    fFunc  = 0;
    fFunc2 = 0;
    fFunc3 = 0;
    ...
    void TPZRandomField::SetYoungField(int distribution, int function){
        fE_dist = distribution;
        fE_funct = function;
    }

    /** @brief Set distribution type and function of nu */
    void TPZRandomField::SetPoissonField(int distribution, int function){
        fnu_dist = distribution;
        fnu_funct = function;
    }
    
    /** @brief Set distribution type: normal or lognormal */
    void TPZRandomField::SetFieldDistribution(const TPZFMatrix<TVar> &M){
        fM = M; //This should be removed later
    }
    
    /** @brief Set inclined parameters if wellbore is inclined */
    virtual void TPZRandomField::SetInclinedField(int stochasticInclined,REAL direction, REAL inclination)
    {
        fstochasticInclined = stochasticInclined;
        fdirection = direction;
        finclination = inclination;
    ...
    virtual void STPZRandomField::etFieldGeometry()
    {
    ...
    virtual void TPZRandomField::InclinedFieldGeometry(){
        int nLayers = 8;
    ...
    TPZFMatrix<TVar> TPZRandomField::EvaluateCorrelation(int function)
    {
        if (fstochasticInclined == 1) {
            
            fK = calcCorrelationMatrixInclined(function);  // Correlation matrix K
            
            //PrintCorrelation();                    // Exporta KCoor .txt
        }
        else{
            
            fK = calcCorrelationMatrix(function);       // Correlation matrix K
    ...
    /** @brief Calculates Correlation either vertical or inclined */
    TPZFMatrix<REAL> TPZRandomField::GetCorrelatedVector(int distribution)
    {
        if (fstochasticInclined == 1) {
            fU = GetDistribution(fmatsize, distribution); // Get random distribution for inclined surface
        }
        else {
            fU = GetDistribution(fnSquareElements, distribution);  // Get random distribution for vertical surfaces
        }
        return fU;
    }
    
    /** @brief Get vector of distribution type */
    TPZFMatrix<TVar> TPZRandomField::GetDistribution(int matrixSize, int distribution)
    {
        //normal distribution
        if(distribution==1) {
    ...
    /** @brief Calculates Correlation either vertical or inclined */
    virtual void TPZRandomField::GetStochasticField( TPZFMatrix<TVar> f_E, TPZFMatrix<TVar> f_nu)
    {
    ...
    TPZFMatrix<REAL> TPZRandomField::calcCorrelationMatrix(int function) {
    ...
    TPZFMatrix<REAL> TPZRandomField::calcCorrelationMatrixInclined(int function) {
    ...
    virtual void TPZRandomField::PrintCorrelation() {
    ...
    TPZVec<TVar> TPZRandomField::calcStochasticField(){
    ...
    virtual int TPZRandomField::NFunctions()
    {
        return 1;
    }
    
    void TPZRandomField::SetPolynomialOrder(int porder)
    {
        fPorder = porder;
    }
    ...
    virtual int TPZRandomField::PolynomialOrder()
    {
#ifdef DEBUG
        if (fPorder == -1) {
            DebugStop();
    ...
    virtual int TPZRandomField::ClassId() const
    {
        return -1;
    }
    
    /** @brief Saves the element data to a stream */
    virtual void TPZRandomField::Write(TPZStream &buf, int withclassid)
    {
        //        DebugStop();
    ...
    virtual void TPZRandomField::Read(TPZStream &buf, void *context)
    {
        DebugStop();
    }
