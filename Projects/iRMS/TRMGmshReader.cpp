...
TPZGeoMesh * TRMGmshReader::GeometricGmshMesh(std::string file_name, TPZGeoMesh *gmesh_input){
    ...
    std::cout << "TRMGmshReader:: Lastest version supported 4.1 " << std::endl;
    std::cout << "TRMGmshReader:: This reader supports only the lastest version " << std::endl;
    std::cout << "TRMGmshReader:: Reader no available for the msh file version = " << m_format_version << std::endl;
    DebugStop();
    return NULL;
}

void TRMGmshReader::PrintPartitionSummary(std::ostream & out){
    
    out << std::endl;
    out << "TRMGmshReader geometrical partition summary " << std::endl;
...
/// Copy constructor
TRMGmshReader::TRMGmshReader(const TRMGmshReader & other){
...
const TRMGmshReader & TRMGmshReader::operator=(const TRMGmshReader & other){
...
TPZGeoMesh * TRMGmshReader::GeometricGmshMesh4(std::string file_name, TPZGeoMesh *gmesh_input){
            ...
            if(str == "$Nodes" || str == "$Nodes\r")
            {
                ...
                for (int64_t i_block = 0; i_block < n_entity_blocks; i_block++)
                {
                    ...
                    if (entity_parametric != 0) {
                        std::cout << "TRMGmshReader:: Characteristic not implemented." << std::endl;
                        DebugStop();
                    }
...
int TRMGmshReader::GetNumberofNodes(int & el_type){
...
TPZGeoMesh * TRMGmshReader::GeometricGmshMesh3(std::string file_name, TPZGeoMesh *gmesh_input)
{
...
void TRMGmshReader::SetCharacteristiclength(REAL length)
{
    m_characteristic_lentgh = length;
}

void TRMGmshReader::SetFormatVersion(std::string format_version)
{
