...
void TRMPhaseTransport::Contribute_abc(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef){
    ...
    TPZFNMatrix<100,STATE> phi_ss_a       = datavec[sb_a].phi;
    TPZFNMatrix<100,STATE> phi_ss_b       = datavec[sb_b].phi;
    REAL sa = datavec[sb_a].sol[0][0];
    REAL sb = datavec[sb_b].sol[0][0];
    
    int nphis_a     = phi_ss_a.Rows();
    int nphis_b     = phi_ss_b.Rows();
    int firsts_a    = 0;
    ...
    // Fluid parameters
    TPZManVector<STATE, 10> rho_w,rho_o,rho_g,Bw_n,Bw,Bo_n,Bo;
    fSimulationData->AlphaProp()->Density(rho_w, v);
    fSimulationData->AlphaProp()->B(Bw, v);
    fSimulationData->AlphaProp()->B(Bw_n, v_n);
    ...
    if (ExplicitSolverQ) {
        if (fSimulationData->IsCurrentStateQ()) {
            for (int is = 0; is < nphis_a; is++)
            {
                ef(is + firsts_a) += weight * (1.0/dt) * phi_n*(1.0/Bw_n[0])* phi_ss_a(is,0);
            }
            for (int is = 0; is < nphis_b; is++)
            {
                ef(is + firsts_b) += weight * (1.0/dt) * phi_n*(1.0/Bo_n[0])* phi_ss_b(is,0);
            }
        }else{
            for (int is = 0; is < nphis_a; is++)
            {
                ef(is + firsts_a) += weight * (1.0/dt) * ( - phi*(sw/Bw[0]) )* phi_ss_a(is,0);
            }
            for (int is = 0; is < nphis_b; is++)
            {
                ef(is + firsts_b) += weight * (1.0/dt) * ( - phi*(so/Bo[0]) )* phi_ss_b(is,0);
            }
        }
    }
    else{
        for (int is = 0; is < nphis_a; is++)
        {
            ...
            ef(is + firsts_a) += weight * (1.0/dt) * (phi_n*(sw_n/Bw_n[0]) - phi*(sw/Bw[0]) )* phi_ss_a(is,0);
            
            for (int js = 0; js < nphis_a; js++)
            {
                ek(is + firsts_a, js + firsts_a) += weight * (1.0/dt) * phi_n*(1.0/Bw_n[0]) * phi_ss_a(js,0) * phi_ss_a(is,0);
            }
        ...
        for (int is = 0; is < nphis_b; is++)
        {
            ...
            ef(is + firsts_b) += weight * (1.0/dt) * (phi_n*(so_n/Bo_n[0]) - phi*(so/Bo[0]) )* phi_ss_b(is,0);
            
            for (int js = 0; js < nphis_b; js++)
            {
                ek(is + firsts_b, js + firsts_b) += weight * (1.0/dt) * phi_n*(1.0/Bo_n[0]) * phi_ss_b(js,0) * phi_ss_b(is,0);
            }
