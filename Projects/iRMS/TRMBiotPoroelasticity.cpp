...
void TRMBiotPoroelasticity::Contribute(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef){
    ...
    if (fSimulationData->ReducedBasisResolution().first) {
        this->Contribute_RB(datavec, weight, ek, ef);
        return;
    }
...
void TRMBiotPoroelasticity::ContributeBC(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef, TPZBndCond &bc){
    ...
    if (fSimulationData->ReducedBasisResolution().first) {
        this->Contribute_RB_BC(datavec, weight, ek, ef, bc);
        return;
    }
...
void TRMBiotPoroelasticity::Contribute_3D(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef){
    ...
    if (fSimulationData->ReducedBasisResolution().first) {
        this->Contribute_RB_3D(datavec, weight, ek, ef);
        return;
    }
...
void TRMBiotPoroelasticity::ContributeBC_3D(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef, TPZBndCond &bc){
    ...
    if (fSimulationData->ReducedBasisResolution().first) {
        this->Contribute_RB_BC_3D(datavec, weight, ek, ef, bc);
        return;
    }
    ...
    switch (bc.Type())
    {
        ...
        case 6 :
        {
            ...
            {
                //	Normal Tension Components on neumman boundary
                ef(3*in,0)		+= 1.0 * weight * Tn[0] * phiu(in,0);		//	Tnx
                ef(3*in+1,0)	+= 1.0 * weight * Tn[1] * phiu(in,0);		//	Tny
...
void TRMBiotPoroelasticity::Contribute_RB(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef){
    ...
    for (int iu = 0; iu < nphi_u; iu++) {
        ...
        // Computing Gradient of the test function for each component
        Grad_vx_i(0,0) = grad_phi_u(0,iu);
        Grad_vx_i(1,0) = grad_phi_u(1,iu);
        Grad_vy_i(0,0) = grad_phi_u(2,iu);
        Grad_vy_i(1,0) = grad_phi_u(3,iu);
        ...
        ef(iu + first_u, 0) += weight * ((S_n(0,0) - source) * Grad_vx_i(0,0) + S_n(0,1) * Grad_vx_i(1,0) - b[0] * phi_u(iu,0));
        ef(iu + first_u, 0)	+= weight * (S_n(1,0) * Grad_vy_i(0,0) + (S_n(1,1) - source) * Grad_vy_i(1,0) - b[1] * phi_u(iu,1));
        
        for (int ju = 0; ju < nphi_u; ju++) {
            
            // Computing Gradient of the test function for each component
            Grad_vx_j(0,0) = grad_phi_u(0,ju);
            Grad_vx_j(1,0) = grad_phi_u(1,ju);
            Grad_vy_j(0,0) = grad_phi_u(2,ju);
            Grad_vy_j(1,0) = grad_phi_u(3,ju);
            ...
            ek(iu + first_u, ju + first_u)  += weight * ( (2.0*mu_dr + l_dr)*duxdx*dvxdx + mu_dr*duxdy*dvxdy);
            ek(iu + first_u, ju + first_u)  += weight * ( l_dr*duydy*dvxdx + mu_dr*duydx*dvxdy);
            ek(iu + first_u, ju + first_u)	+= weight * ( l_dr*duxdx*dvydy + mu_dr*duxdy*dvydx);
            ek(iu + first_u, ju + first_u)	+= weight * ( (2.0*mu_dr + l_dr)*duydy*dvydy + mu_dr*duydx*dvydx);
...
void TRMBiotPoroelasticity::Contribute_RB_BC(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef, TPZBndCond &bc){
    ...
    switch (bc.Type())
    {
        case 0 :
        {
            ...
            {
                //	Contribution for load Vector
                ef(in,0)	+= gBigNumber*(u[0] - v[0])*phiu(in,x)*weight;	// X displacement Value
                ef(in,0)    += gBigNumber*(u[1] - v[1])*phiu(in,y)*weight;	// y displacement Value
                
                for (jn = 0 ; jn < phru; jn++)
                {
                    //	Contribution for Stiffness Matrix
                    ek(in,jn)	+= gBigNumber*phiu(in,x)*phiu(jn,x)*weight;	// X displacement
                    ek(in,jn)	+= gBigNumber*phiu(in,y)*phiu(jn,y)*weight;	// Y displacement
                }
            }
            break;
        ...
        case 3 :
        {
            ...
            {
                //	Normal Tension Components on neumman boundary
                ef(in,0)	+= -1.0 * weight * v[0] * phiu(in,x);		//	Tnx
                ef(in,0)	+= -1.0 * weight * v[1] * phiu(in,y);		//	Tny
            }
        ...
        case 4 :
        {
            ...
            {
                //	Normal Tension Components on neumman boundary
                ef(in,0)		+= -1.0* v[0]*phiu(in,x)*weight;		//	Tnx
            }
        ...
        case 5 :
        {
            ...
            {
                //	Normal Tension Components on neumman boundary
                ef(in,0)	+= -1.0* v[1]*phiu(in,y)*weight;		//	Tny
            }
...
void TRMBiotPoroelasticity::Contribute_RB_3D(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef){
    ...
    for (int iu = 0; iu < nphi_u; iu++) {
        ...
        Grad_vx_i(0,0) = grad_phi_u(0,iu);
        Grad_vx_i(1,0) = grad_phi_u(1,iu);
        Grad_vx_i(2,0) = grad_phi_u(2,iu);
        
        Grad_vy_i(0,0) = grad_phi_u(3,iu);
        Grad_vy_i(1,0) = grad_phi_u(4,iu);
        Grad_vy_i(2,0) = grad_phi_u(5,iu);
        
        Grad_vz_i(0,0) = grad_phi_u(6,iu);
        Grad_vz_i(1,0) = grad_phi_u(7,iu);
        Grad_vz_i(2,0) = grad_phi_u(8,iu);
        
        ef(iu + first_u, 0) += weight * ((S_n(0,0) - source) * Grad_vx_i(0,0) + S_n(0,1) * Grad_vx_i(1,0) + S_n(0,2) * Grad_vx_i(2,0) - b[0] * phi_u(iu,0));
        ef(iu + first_u, 0)	+= weight * (S_n(1,0) * Grad_vy_i(0,0) + (S_n(1,1) - source) * Grad_vy_i(1,0) + S_n(1,2) * Grad_vy_i(2,0) - b[1] * phi_u(iu,1));
        ef(iu + first_u, 0)	+= weight * (S_n(2,0) * Grad_vz_i(0,0) + S_n(2,1) * Grad_vz_i(1,0) + (S_n(2,2) - source) * Grad_vz_i(2,0) - b[2] * phi_u(iu,2));
        ...
        for (int ju = 0; ju < nphi_u; ju++) {
            ...
            Grad_vx_j(0,0) = grad_phi_u(0,ju);
            Grad_vx_j(1,0) = grad_phi_u(1,ju);
            Grad_vx_j(2,0) = grad_phi_u(2,ju);
            
            Grad_vy_j(0,0) = grad_phi_u(3,ju);
            Grad_vy_j(1,0) = grad_phi_u(4,ju);
            Grad_vy_j(2,0) = grad_phi_u(5,ju);
            
            Grad_vz_j(0,0) = grad_phi_u(6,ju);
            Grad_vz_j(1,0) = grad_phi_u(7,ju);
            Grad_vz_j(2,0) = grad_phi_u(8,ju);
            ...
            // Gradient 1
            ek(iu + first_u, ju + first_u)  += weight * ((l_dr + 2.*mu_dr)*duxdx*dvxdx + mu_dr*duxdy*dvxdy + mu_dr*duxdz*dvxdz);
            ek(iu + first_u, ju  + first_u) += weight * (l_dr*duydy*dvxdx + mu_dr*duydx*dvxdy);
            ek(iu + first_u, ju  + first_u) += weight * (l_dr*duzdz*dvxdx + mu_dr*duzdx*dvxdz);
            
            // Gradient 2
            ek(iu + first_u, ju  + first_u) += weight * (l_dr*duxdx*dvydy + mu_dr*duxdy*dvydx);
            ek(iu + first_u, ju  + first_u) += weight * ((l_dr + 2.*mu_dr)*duydy*dvydy + mu_dr*duydx*dvydx + mu_dr*duydz*dvydz);
            ek(iu + first_u, ju  + first_u) += weight * (l_dr*duzdz*dvydy + mu_dr*duzdy*dvydz);
            
            // Gradient 3
            ek(iu + first_u, ju  + first_u) += weight * (l_dr*duxdx*dvzdz + mu_dr*duxdz*dvzdx);
            ek(iu + first_u, ju  + first_u) += weight * (l_dr*duydy*dvzdz + mu_dr*duydz*dvzdy);
            ek(iu + first_u, ju  + first_u) += weight * ((l_dr + 2.*mu_dr)*duzdz*dvzdz + mu_dr*duzdx*dvzdx + mu_dr*duzdy*dvzdy);
...
void TRMBiotPoroelasticity::Contribute_RB_BC_3D(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef, TPZBndCond &bc){
    ...
    switch (bc.Type())
    {
        case 0 :
        {
            ...
            for(in = 0 ; in < phru; in++)
            {
                ...
                ef(in,0)	+= gBigNumber*(u[0] - v[0])*phiu(in,x)*weight;	// X displacement Value
                ef(in,0)	+= gBigNumber*(u[1] - v[1])*phiu(in,y)*weight;	// y displacement Value
                ef(in,0)	+= gBigNumber*(u[2] - v[2])*phiu(in,z)*weight;	// Z displacement Value
                ...
                for (jn = 0 ; jn < phru; jn++)
                {
                    ...
                    ek(in,jn)	+= gBigNumber*phiu(in,x)*phiu(jn,x)*weight;	// X displacement
                    ek(in,jn)	+= gBigNumber*phiu(in,y)*phiu(jn,y)*weight;	// Y displacement
                    ek(in,jn)	+= gBigNumber*phiu(in,z)*phiu(jn,z)*weight;	// Z displacement
                }
            }
            break;
        ...
        case 3 :
        {
            ...
            for(in = 0 ; in < phru; in++)
            {
                ...
                ef(in,0)	+= -1.0 * weight * v[0] * phiu(in,x);		//	Tnx
                ef(in,0)	+= -1.0 * weight * v[1] * phiu(in,y);		//	Tny
                ef(in,0)	+= -1.0 * weight * v[2] * phiu(in,z);		//	Tnz
            }
        ...
        case 6 :
        {
            ...
            for(in = 0 ; in < phru; in++)
            {
                ...
                ef(in,0)	+= 1.0 * weight * Tn[0] * phiu(in,x);		//	Tnx
                ef(in,0)	+= 1.0 * weight * Tn[1] * phiu(in,y);		//	Tny
                ef(in,0)	+= 1.0 * weight * Tn[2] * phiu(in,z);		//	Tnz
            }
