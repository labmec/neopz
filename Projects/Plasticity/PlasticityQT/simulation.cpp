...
void TPZPlasticityTest::ApplyInitialStress()
{
    ...
    qDebug() << "----------------------------> " << I1Initial << I1 << fSandler.fYC.fA << fSandler.fYC.fR << fPoreStressRZ[0] << fPoreStressRZ[1];
    // should improve!!
...
void TPZPlasticityTest::LoadInputStrainStress ( QVector<double> *sigmaAxialTotal, QVector<double> *sigmaLateral,
                                                QVector<double> *defAxial, QVector<double> *defLateral) {

//    qDebug() << "VALORES DA TELA: " <<sigmaAxialTotal->size() <<sigmaLateral <<defAxial <<defLateral;
...
//        qDebug() << "i=" << i << fStressRZInput(i,1) << fStressRZInput(i,0) << fStrainRZInput(i,1)<< fStrainRZInput(i,0);
    }
