...
/*
int mainCube(int argc, char * const argv[])
{
	...
	int elId = 0;
	TPZVec <int> Topol(20);
    for(int n = 0; n < 20; n++) Topol[n] = n;
    new TPZGeoElRefPattern< TPZQuadraticCube > (elId,Topol,matElId,*gmesh);
    elId++;
...
//    gRefDBase.InitializeUniformRefPattern(EPiramide);
    ...
    nodeId++;
    //
    NodeCoord[nodeId][0] = +0.5-d;
    NodeCoord[nodeId][1] = +0.+d;
    NodeCoord[nodeId][2] = -1.-d;
    nodeId++;

    NodeCoord[nodeId][0] = +0.5+d;
    NodeCoord[nodeId][1] = +0.5-d;
    NodeCoord[nodeId][2] = -1.+d;
    nodeId++;
    ...
    NodeCoord[nodeId][0] = +0.5;
    NodeCoord[nodeId][1] = +0.5;
    NodeCoord[nodeId][2] = +1.;
    nodeId++;
	...
	int elId = 0;
	TPZVec <int> Topol(15);
    for(int n = 0; n < 15; n++) Topol[n] = n;
    new TPZGeoElRefPattern< TPZQuadraticPrism > (elId,Topol,matElId,*gmesh);
    elId++;
    ...
    std::cout << "Summ of phi's at many points:\n";
    int npts = 4;
    TPZQuadraticPrism * myPrism = dynamic_cast<TPZQuadraticPrism*>(myGel);
    TPZFMatrix phi(myGel->NNodes(),1), dphi(myGel->Dimension(),myGel->NNodes());
    for(int xi = 0; xi <= npts; xi++)
    {
        for(int et = 0; et <= npts; et++)
...
/*
 int mainPyramid(int argc, char * const argv[])
 {
 //    gRefDBase.InitializeUniformRefPattern(ECube);
 //    gRefDBase.InitializeUniformRefPattern(EPiramide);
 ...
 TPZVec <int> Topol(20);
 for(int n = 0; n < 20; n++) Topol[n] = n;
 new TPZGeoElRefPattern< TPZQuadraticPyramid > (elId,Topol,matElId,*gmesh);
 elId++;
 ...
 std::cout << "Summ of phi's at many points:\n";
 int npts = 4;
 TPZQuadraticPyramid * myPyramid = dynamic_cast<TPZQuadraticPyramid*>(myGel);
 TPZFMatrix phi(myGel->NNodes(),1), dphi(myGel->Dimension(),myGel->NNodes());
 for(int xi = 0; xi <= npts; xi++)
 {
 for(int et = 0; et <= npts; et++)
