...
void TSWXGraphMeshSol::AddSol(double time, const swx::vector< TSWXGraphSol >  &sol){
...
void TSWXGraphMeshSol::GetSolData(int iStep, double &time, swx::vector< TSWXGraphSol >  &sol) const{
  sol = GetSolData(iStep,time);
}

const swx::vector< TSWXGraphSol >  & TSWXGraphMeshSol::GetSolData(int iStep, double &time) const{
  if(iStep < 0 || iStep >= this->NTimeSteps()){
    DebugStop();
  }

  std::map< double, swx::vector< TSWXGraphSol > >::const_iterator w = this->fAllSol.begin();
...
void TSWXGraphMeshSol::Read(std::istream & file){
  ...
  for(int i = 0; i < n; i++){
    double time;
    file >> time;
    swx::vector< TSWXGraphSol > sol;
    swx::ReadVec(file, sol);
    fAllSol[time] = sol;
  }
}
...
void TSWXGraphMeshSol::Write(std::ostream &file) const{
	...
	file << fAllSol.size() << "\n";
	std::map< double, swx::vector< TSWXGraphSol > >::const_iterator w;
	for(w = fAllSol.begin(); w != fAllSol.end(); w++){
		file << w->first << "\n";
		swx::WriteVec( file, w->second );
	}
}
swx::vector<std::string> TSWXGraphMeshSol::GetSolutionsNames() const
{
	swx::vector<std::string> names(fAllSol.begin()->second.size());
...
{
	std::map< double, swx::vector< TSWXGraphSol > >::const_iterator it;
	int solId = -1;
	for(int i = 0; i < fAllSol.begin()->second.size(); i++)
	{
		solId = i;
...
bool TSWXGraphMesh::HasNodalSolution() const{
  if(fSol.NTimeSteps()){
    double time;
    const swx::vector< TSWXGraphSol > & sol = fSol.GetSolData(0,time);
...
void TSWXGraphMesh::ToParaview(std::ostream &file, int mathStyleIStep) const{
  ...
  if(this->HasNodalSolution()){
    ...
    double time;
    const swx::vector< TSWXGraphSol > & sol = fSol.GetSolData(iStep,time);
