...
// copiado do Cedric
   ...
   REAL minx=20., miny=20., maxx=-10., maxy=-10.;
   for(int ii=0;ii<gElem.NElements();ii++)
	   for(int jj=0;jj<gElem[ii]->NSides()-1;jj++) {
			gElem[ii]->CenterPoint(jj,center);
			gElem[ii]->X(center,point);
   ...
   for(int ii=0;ii<gElem.NElements();ii++)
	   for(int jj=gElem[ii]->NCornerNodes();jj<gElem[ii]->NSides()-1;jj++) {
			gElem[ii]->CenterPoint(jj,center);
			gElem[ii]->X(center,point);
			if(point[0]==minx)
			   TPZGeoElBC((TPZGeoEl *)gElem[ii],jj,-4);
			if(point[0]==maxx)
			   TPZGeoElBC((TPZGeoEl *)gElem[ii],jj,-2);
			if(point[1]==miny)
			   TPZGeoElBC((TPZGeoEl *)gElem[ii],jj,-1);
			if(point[1]==maxy)
			   TPZGeoElBC((TPZGeoEl *)gElem[ii],jj,-3);
	   }
...
//   TPZGeoElBC((TPZGeoEl *)gElem[8],6,-3);
   bc = mat->CreateBC(mat,-3,0,val1,val2);
   cmesh->InsertMaterialObject(bc);
...
//   TPZGeoElBC((TPZGeoEl *)gElem[4],7,-4);
   bc = mat->CreateBC(mat,-4,0,val1,val2);
   cmesh->InsertMaterialObject(bc);

   cmesh->SetDimModel(dim);					// 2017 KKK
   cmesh->AutoBuild();
...
//   for(j=0;j<Solution.Rows();j+=nShape)
   {
      int blockOffset = cmesh->Block().Position(j);
	  for(int kk=0;kk<nShape;kk++) {
...
//      Solution(j+1,0) = ro * u;
//      Solution(j+2,0) = ro * v;
//      Solution(j+3,0) = p/(gamma-1.0) + 0.5 * ro * vel2;
/*
