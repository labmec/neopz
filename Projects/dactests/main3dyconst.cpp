...
TPZGeoMesh *CreateOneCuboWithTetraedronsYconst(long nelem=1, int MaterialId=1);
...
TPZGeoMesh * BasicForm(int n, REAL t, REAL dt){
    ...
    TPZAutoPointer<TPZFunction<STATE> > ParFunc3 = new TPZDummyFunction<REAL>(Parametricfunction3);
...
TPZGeoMesh *CreateOneCuboWithTetraedronsYconst(long nelem, int MaterialId)
{
    TPZGeoMesh *gmesh = new TPZGeoMesh;
...
TPZCompMesh *CMeshMixedYconst(TPZGeoMesh * gmesh, TPZVec<TPZCompMesh *> meshvec)
{
    ...
    TPZDummyFunction<STATE> *dum = new TPZDummyFunction<STATE>(ForcingYconst);
    ...
    val2(0,0) = 0.0;
    val2(1,0) = 0.0;
    TPZAutoPointer<TPZFunction<STATE> > FBCond2 = new TPZDummyFunction<STATE>(ForcingBC2DYconst);
...
void ErrorHDivYconst(TPZCompMesh *hdivmesh, std::ostream &out, int p, int ndiv)
{
    ...
    for (long el=0; el<nel; el++) {
        ...
        elerror.Fill(0.);
        cel->EvaluateError(SolExataYconst, elerror, NULL);
