...
void ToolsTransient::ElastNLTestWithCohesive()
{
  ...
  TPZCompMesh *cmesh = this->CreateCMeshCohe(gmesh);
...
TPZGeoMesh* ToolsTransient::CreateGeoMeshCohe()
{
  ...
  long index = 0;
  TPZGeoEl *gel = NULL;
  gel = gmesh->CreateGeoElement(EQuadrilateral, TopolQuad, globReservMatId1, index);
  gel->SetId(index);
  index++;
...
TPZCompMesh* ToolsTransient::CreateCMeshCohe(TPZGeoMesh *gmesh)
{
  TPZCompMesh *cmesh = new TPZCompMesh(gmesh);
  /// criar materiais
	int dim = 2;
...
void ToolsTransient::SolveLinearElasticity(TPZCompMesh *cmesh)
{
...
TPZGeoMesh* ToolsTransient::CreateGeoMesh()
{
  TPZGeoMesh *gmesh = new TPZGeoMesh;
  int nnodes = 4, nodeid = 0;
...
TPZCompMesh* ToolsTransient::CreateCMesh(TPZGeoMesh *gmesh)
{
  TPZCompMesh *cmesh = new TPZCompMesh(gmesh);
  /// criar materiais
	int dim = 2;
