...
REAL FluxoSaida(TPZAutoPointer<TPZCompMesh> cmesh, TPZFMatrix &solution, int bc);
...
//    TPZGenGrid(TPZVec<int> &nx, TPZVec<REAL> &x0, TPZVec<REAL> &x1, int numl = 1, REAL rot = 0.5);
    ...
    g_Length = L;
    bc2->SetForcingFunction(forcingfunction);
    TPZAutoPointer<TPZMaterial> bcauto2(bc2);
    bcauto2->SetForcingFunction(forcingfunction);
...
REAL FluxoSaida(TPZAutoPointer<TPZCompMesh> cmesh, TPZFMatrix &solution, int bc)
{
    ...
    TPZFMatrix rhs;
    TPZAutoPointer<TPZMatrix> mat = strskyl.CreateAssemble(rhs, gui);
