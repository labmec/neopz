/* Generated by Together */

#include "pzadaptmesh.h"
#include "pzgclonemesh.h"
#include "pzcclonemesh.h"
#include "pzgeoel.h"
#include "pzcompel.h"
#include "pzfstrmatrix.h"
#include "pzstepsolver.h"
#include "pzintel.h"
#include "pzquad.h"
#include "pzonedref.h"
#include "pzcheckmesh.h"
//multithread -->>
#include "pz_pthread.h"
#include <signal.h>

int TPZAdaptMesh::fThreads_in_use = 0;
int TPZAdaptMesh::fNClones_to_Analyse = 0;
pthread_cond_t TPZAdaptMesh::fSignal_free = PTHREAD_COND_INITIALIZER;
pthread_mutex_t TPZAdaptMesh::fLock_clindex = PTHREAD_MUTEX_INITIALIZER;

TPZAdaptMesh::TPZAdaptMesh(TPZCompMesh *mesh){
  fReference = mesh;
  int nel = fReference->ElementVec().NElements();
  fElementError.Resize(nel);
  fGeoRef.Resize(0);
  fPatch.Resize(0);
  fPatchIndex.Resize(0);
  fMaxP = 10;

  fTrueErrorVec.Resize(nel);

  fElementError.Fill(0.);
  fTrueErrorVec.Fill(0.);

  //changed to use multi-threading
  fTrueErrorVec.Resize(nel);
  fTrueErrorVec.Fill(0.);
  fExact = 0;
  fClonestoAnalyse.Resize(0);
  //gets the geometric reference elements that will generate the patch
  BuildReferenceElements();
  //Generates the patch
  BuildReferencePatch();
}

TPZAdaptMesh::~TPZAdaptMesh(){
  CleanUp();
}

void TPZAdaptMesh::SetMaxP(int maxp){
  if (maxp < 1) {
    cout << "TPZAdaptMesh::Error : SetMaxP - maximum p order must be greter than 0... trying to set maximum p to "
	 << maxp << endl;
    return;
  }
  fMaxP = maxp;
  TPZOneDRef::gMaxP = fMaxP;
}

void TPZAdaptMesh::CleanUp(){
  int ncl = fCloneStructs.NElements(),i;
  for(i=0; i<ncl; i++) fCloneStructs[i].Cleanup();
}

void TPZAdaptMesh::TPZCloneInfo::Cleanup() {
  if(!fCloneMesh) return;
  TPZGeoCloneMesh *gmesh = dynamic_cast<TPZGeoCloneMesh *> (fCloneMesh->Reference());
  gmesh->ResetReference();
  //Cesar July 2003 ->
  //If some reference element is not used to analyse error its fine clone mesh is not created!
  if (fFineCloneMesh){
    fFineCloneMesh->LoadReferences();
    RemoveCloneBC(fFineCloneMesh);
    DeleteElements(fFineCloneMesh);
    delete fFineCloneMesh;
    fFineCloneMesh = 0;
  }
  //<-
  fCloneMesh->LoadReferences();
  RemoveCloneBC(fCloneMesh);
  DeleteElements(fCloneMesh);
  delete fCloneMesh;
  delete gmesh;
  fCloneMesh = 0;
}


void TPZAdaptMesh::GetAdaptedMesh(TPZStack<TPZGeoEl *> &gelstack, TPZStack<int> &porder,
                                  REAL &error, REAL & truerror, TPZVec<REAL> &ervec,
                                  void(*f)(TPZVec<REAL> &loc, TPZVec<REAL> &val, TPZFMatrix &deriv),
                                  TPZVec<REAL> &truervec,
                                  TPZVec<REAL> &effect,int use_trueerror){

  int nelmesh = fReference->ElementVec().NElements();
  effect.Resize(nelmesh);
  truervec.Resize(nelmesh);
  ervec.Resize(nelmesh);
  ervec.Fill(0.);
  truervec.Fill(0.);
  effect.Fill(0.);
  if (f) fExact = f;
  else fExact = 0;

  //Creates the patch clones;
  fReference->ComputeNodElCon();

  int printing = 0;
  if(printing) {
    ofstream test("test.txt",ios::app);
    fReference->Print(test);
    fReference->Reference()->Print(test);
  }
  int ncl = NPatches();

  //clone analysis
  int cliter;
  //Used to evaluate the error when the true solution is provided======
  TPZVec<int> perm(nelmesh,0);
  int i;
  for(i=0; i<nelmesh; i++) {
    perm[i] = i;
  }
  REAL minerror = 0.;
  //===================================================================
  if (use_trueerror && f){
    for (i=0;i<fReference->ElementVec().NElements();i++){
      TPZInterpolatedElement *el = dynamic_cast<TPZInterpolatedElement *> (fReference->ElementVec()[i]);
      if (el)
        fTrueErrorVec[i] = ComputeTrueError (el,f);
      else
        fTrueErrorVec[i] = 0.;
    }
    minerror = SortMinError(fTrueErrorVec,perm,0.65);
    cout << "MinError " << minerror << endl;
  }

  //Creates an uniformly refined mesh and evaluates the error
  for (cliter = 0; cliter<ncl; cliter++){
     fClonestoAnalyse.Push(cliter);
  }

  int nthreads = 0;
  fNClones_to_Analyse = fClonestoAnalyse.NElements();
  pthread_t *allthreads = new pthread_t[(const int)fNClones_to_Analyse];
  while (fNClones_to_Analyse) {
    if (fThreads_in_use <=  nthreads){
      PZ_PTHREAD_MUTEX_LOCK(&fLock_clindex,"TPZAdaptMesh::GetAdaptedMesh()");
      int thr_index = fNClones_to_Analyse-1;
      fNClones_to_Analyse --;
      fThreads_in_use ++;
      int result =   PZ_PTHREAD_CREATE(&allthreads[thr_index],NULL,(void*(*)(void*))TPZAdaptMesh::MeshError,this,"TPZAdaptMesh::GetAdaptedMesh()");
      if (result){
        cout << "TPZAdaptMesh::GetAdaptMesh : Error on thread creation... exiting\n" << result << "\t "<< EAGAIN << endl;
        exit (-1);
      }
    }
    PZ_PTHREAD_COND_WAIT(&fSignal_free,&fLock_clindex,"TPZAdaptMesh::GetAdaptedMesh()");
    PZ_PTHREAD_MUTEX_UNLOCK(&fLock_clindex,"TPZAdaptMesh::GetAdaptedMesh()");
  }

  for(i=0;i<fClonestoAnalyse.NElements();i++){
    PZ_PTHREAD_JOIN(allthreads[i],0,"TPZAdaptMesh::GetAdaptedMesh()");
  }
  delete[] allthreads;
  truervec = fTrueErrorVec;

  //Ordena o vetor de erros
  int nelem = fElementError.NElements();
  //  TPZVec<int> perm(nelem,0);
  error = 0.;
  for(i=0; i<nelem; i++) {
    perm[i] = i;
    ervec[i]=fElementError[i];
    error += fElementError[i];
  }
  if(!use_trueerror || !f) {
    minerror = SortMinError(fElementError,perm,0.65);
    cout << "MinError " << minerror << endl;
  }
  truerror = 0.;
  if(f) {
    for(i=0; i<nelem; i++){
      truerror += fTrueErrorVec[i];
    }
  }

  //inicializa effect com o tamanho de trueeerror
  if(f) {
    for(i=0; i<nelem; i++) {
      if(fTrueErrorVec[i] >= 1.e-4*truerror && fTrueErrorVec[i] >= 5e-20 ) {
        effect[i] = fElementError[i]/fTrueErrorVec[i];
      }
      else {
        effect[i]=0.;
        fTrueErrorVec[i]=0.;
      }
    }
  }
  //  int nstate = fCloneMesh[0]->MaterialVec()[0]->NStateVariables();

  int ibc;
  for (ibc=0;ibc<fReference->ElementVec().NElements();ibc++){
    TPZCompEl *cel = fReference->ElementVec()[ibc];
    if(!cel) continue;
    TPZInterpolatedElement *cintel = dynamic_cast<TPZInterpolatedElement *> (cel);
    int matid =cintel->Material()->Id();
    if ( matid < 0){
      int cintorder = cintel->PreferredSideOrder(cintel->NConnects() -1);
      gelstack.Push(cintel->Reference());
      porder.Push(cintorder);
    }
  }


  //Analyse clone element error and, if necessary, analyse element and changes its refinement pattern
  for (i=0;i<fCloneStructs.NElements();i++){
//    fCloneStructs[i].PrintElementIds(cout);
//    cout << endl;
    fCloneStructs[i].AddSelectedElements(fElementError,minerror,gelstack,porder);
  }
  int nelg = gelstack.NElements();
  int g;
  for(g=0; g<nelg; g++) {
    cout << gelstack[g]->Id() << "/" << porder[g] << " ";
  }
  cout << endl;
}

void * TPZAdaptMesh::MeshError(void *t){
  TPZAdaptMesh *adapt = (TPZAdaptMesh *) (t);
  PZ_PTHREAD_MUTEX_LOCK(&adapt->fLock_clindex,"TPZAdaptMesh::MeshError()");
  int cliter = adapt->fClonestoAnalyse[adapt->fNClones_to_Analyse];
//   TPZGeoCloneMesh *gcmesh = dynamic_cast<TPZGeoCloneMesh *> (adapt->fCloneMesh[cliter]->Reference());
//  adapt->fFineCloneMesh [cliter] = adapt->fCloneMesh[cliter]->UniformlyRefineMesh();
//   if (gcmesh->ReferenceElement(0)->MaterialId() <  0){
//     pthread_cond_signal (&adapt->fSignal_free);
//     pthread_mutex_unlock(&adapt->fLock_clindex);
//     pthread_mutex_lock(&adapt->fLock_clindex);
//     fThreads_in_use --;
//     pthread_mutex_unlock(&adapt->fLock_clindex);
//     return 0;
//   }
  PZ_PTHREAD_COND_SIGNAL(&adapt->fSignal_free,"TPZAdaptMesh::MeshError()");
  PZ_PTHREAD_MUTEX_UNLOCK(&adapt->fLock_clindex,"TPZAdaptMesh::MeshError()");
  if(adapt->fGeoRef[cliter]->MaterialId() >= 0) {
    adapt->ProcessPatch(cliter);
  }
  PZ_PTHREAD_MUTEX_LOCK(&adapt->fLock_clindex,"TPZAdaptMesh::MeshError()");
  fThreads_in_use --;
  PZ_PTHREAD_COND_SIGNAL(&adapt->fSignal_free,"TPZAdaptMesh::MeshError()");
  PZ_PTHREAD_MUTEX_UNLOCK(&adapt->fLock_clindex,"TPZAdaptMesh::MeshError()");
  return 0;
}

void TPZAdaptMesh::ProcessPatch(int icl) {
  if(fGeoRef[icl]->MaterialId() < 0) return;
  TPZCompCloneMesh *coarse = 0;
  TPZCompMesh *fine = 0;
  PZ_PTHREAD_MUTEX_LOCK(&fLock_clindex, "TPZAdaptMesh::ProcessPatch()");
  coarse = fCloneStructs[icl].fCloneMesh;
  if(! coarse) {
    CreatePatch(icl);
    coarse =  fCloneStructs[icl].fCloneMesh;
  }
  fine = fCloneStructs[icl].fFineCloneMesh;
  if(! fine) {
    fCloneStructs[icl].fFineCloneMesh = fCloneStructs[icl].fCloneMesh->UniformlyRefineMesh();
    fine = fCloneStructs[icl].fFineCloneMesh;
  }
  PZ_PTHREAD_MUTEX_UNLOCK(&fLock_clindex, "TPZAdaptMesh::ProcessPatch()");
  coarse->MeshError( fine,fElementError,fExact,fTrueErrorVec);
  coarse->ComputeRefPattern(fine, fCloneStructs[icl].fRefinedElements,fCloneStructs[icl].fRefinedOrders);
  fCloneStructs[icl].Cleanup();
}

void TPZAdaptMesh::BuildReferenceElements(){
  if (!fReference){
    cout << "TPZAdaptMesh::Error:\n computational mesh must be initialized to call GetReferenceElements!\n";
    return;
  }
  fReference->GetRefPatches(fGeoRef);
  fCloneStructs.Resize(fGeoRef.NElements());

/*   int i; */
/*   int nel = fGeoRef.NElements(); */
/*   for (i=0;i<nel; i++){ */
/*     fGeoRef[i]->Print(cout); */
/*   } */

}

void TPZAdaptMesh::BuildReferencePatch(){
  TPZGeoMesh *gmesh = fReference->Reference();
  gmesh->ResetReference();
  TPZCompMesh *tmpcmesh = new TPZCompMesh (gmesh);
  int i,j;
  fReference->CopyMaterials(tmpcmesh);
  for (i=0;i<fGeoRef.NElements();i++){
    fGeoRef[i]->CreateCompEl(*tmpcmesh,i);
  }
  tmpcmesh->CleanUpUnconnectedNodes();
  TPZStack <int> patchelindex;
  TPZStack <TPZGeoEl *> toclonegel;
  TPZVec<int> n2elgraph;
  TPZVec<int> n2elgraphid;
  TPZStack<int> elgraph;
  TPZVec<int> elgraphindex;
  tmpcmesh->GetNodeToElGraph(n2elgraph,n2elgraphid,elgraph,elgraphindex);
  int clnel = tmpcmesh->NElements();
  fPatchIndex.Push(0);
  for (i=0; i<clnel; i++){
    tmpcmesh->GetElementPatch(n2elgraph,n2elgraphid,elgraph,elgraphindex,i,patchelindex);
    for (j=0; j<patchelindex.NElements(); j++){
      TPZGeoEl *gel = tmpcmesh->ElementVec()[patchelindex[j]]->Reference();
      //      int count = 0;
      if(gel) fPatch.Push(gel);
    }
    int sum = fPatch.NElements();
    fPatchIndex.Push(sum);
  }
  gmesh->ResetReference();
  delete tmpcmesh;
  fReference->LoadReferences();
}

void TPZAdaptMesh::CreatePatch(int clid){
  fReference->Reference()->ResetReference();
  fReference->LoadReferences();
  TPZGeoMesh *geomesh = fReference->Reference();

  TPZStack<TPZGeoEl*> patch;

  int elid;
  TPZGeoCloneMesh *geoclone = new TPZGeoCloneMesh(geomesh);
  for (elid=fPatchIndex[clid];elid<fPatchIndex[clid+1];elid++){
    patch.Push(fPatch[elid]);
  }
  geoclone->SetElements(patch,fGeoRef[clid]);

  TPZVec<TPZGeoEl *> sub;
  //    int ngcel = geoclone->ElementVec().NElements();
  int printing = 0;
  if(printing) {
    ofstream out("test.txt",ios::app);
    geoclone->Print(out);
  }
  TPZCompCloneMesh *clonecompmesh = new TPZCompCloneMesh(geoclone,fReference);
  clonecompmesh->AutoBuild();
  fCloneStructs[clid].fCloneMesh = clonecompmesh;

  // Fill in the structure of the original elements
  int nelpatch = clonecompmesh->NElements();
  for(elid=0; elid<nelpatch; elid++) {
    TPZCompEl *cel = clonecompmesh->ElementVec()[elid];
    if(cel->Material()->Id() < 0) continue;
    TPZGeoEl *gel = cel->Reference();
    if(clonecompmesh->IsFather(gel)) {
      int origindex = clonecompmesh->GetOriginalElementIndex(cel->Index());
      if(origindex >= 0) {
        TPZCompEl *celorig = fReference->ElementVec()[origindex];
        TPZInterpolatedElement *cintel = dynamic_cast<TPZInterpolatedElement *>(celorig);
        fCloneStructs[clid].fOriginalElements.Push(cintel->Reference());
        int nc = cintel->NConnects();
        int ord = cintel->PreferredSideOrder(nc-1);
        fCloneStructs[clid].fOriginalOrders.Push(ord);
      }
    }
  }
}


void TPZAdaptMesh::Sort(TPZVec<REAL> &vec, TPZVec<int> &perm) {
  int i,j;
  int imin = 0;
  int imax = vec.NElements();
  for(i=imin; i<imax; i++) {
    for(j=i+1; j<imax; j++) {
      if(vec[perm[i]] < vec[perm[j]]) {
        int kp = perm[i];
        perm[i] = perm[j];
        perm[j] = kp;
      }
    }
  }
}

void TPZAdaptMesh::HeapSort(TPZVec<REAL> &sol, TPZVec<int> &perm){
  int nelem = perm.NElements();
  int i,j;
  for(i=0; i<nelem; i++) perm[i] = i;

  if(nelem == 1) return;
  int l, ir,ind;
  REAL q;
  l= nelem/2;
  ir = nelem-1;
  while(l>0 && ir>0) {
    if(l> 0) {
      l--;
      ind = perm[l];
      q=sol[ind];
    } else {
      ind = perm[ir];
      q = sol[ind];
      perm[ir] = perm[0];
      ir--;
    }
    i=l;
    j=l+l+1;
    while(j<=ir) {
      if(j<ir && sol[perm[j]] < sol[perm[j+1]]) j++;
      if(q < sol[perm[j]]) {
        perm[i] = perm[j];
        i=j;
        j= i+i+1;
      } else {
        break;
      }
    }
    perm[i] = ind;
  }
}

TPZCompMesh *TPZAdaptMesh::CreateCompMesh (TPZCompMesh *mesh,             //malha a refinar
					   TPZVec<TPZGeoEl *> &gelstack,  //
					   TPZVec<int> &porders) {

  //Cria um ponteiro para a malha geométrica de mesh
  TPZGeoMesh *gmesh = mesh->Reference();
  if(!gmesh) {
    cout << "TPZAdaptMesh::CreateCompMesh encountered no geometric mesh\n";
    return 0;
  }

  //Reseta as referências do ponteiro para a malha geométrica criada
  //e cria uma nova malha computacional baseada nesta malha geométrica
  gmesh->ResetReference();
  TPZCompMesh *cmesh = new TPZCompMesh(gmesh);

#ifdef HUGE_DEBUG
  TPZCheckMesh check(cmesh,&cout);
#endif

  int nmat = mesh->MaterialVec().NElements();
  int m;

  //Cria um clone do vetor de materiais da malha mesh
  for(m=0; m<nmat; m++) {
    TPZMaterial *mat = mesh->MaterialVec()[m];
    if(!mat) continue;
    mat->Clone(cmesh->MaterialVec());
  }

  //Idenifica o vetor de elementos computacionais de mesh
  //  TPZAdmChunkVector<TPZCompEl *> &elementvec = mesh->ElementVec();

  int el,nelem = gelstack.NElements();
  //  cmesh->SetName("Antes PRefine");
  //  cmesh->Print(cout);

  for(el=0; el<nelem; el++) {
    //identifica os elementos geométricos passados em gelstack
    TPZGeoEl *gel = gelstack[el];
    if(!gel) {
      cout << "TPZAdaptMesh::CreateCompMesh encountered an null element\n";
      continue;
    }
    int celindex;
    //Cria um TPZIntel baseado no gel identificado
    int temporder = cmesh->GetDefaultOrder();
    cmesh->SetDefaultOrder(porders[el]);
    TPZInterpolatedElement *csint;
    csint = dynamic_cast<TPZInterpolatedElement *> (gel->CreateCompEl(*cmesh,celindex));
    cmesh->SetDefaultOrder(temporder);

#ifdef HUGE_DEBUG
    if(check.CheckConnectOrderConsistency() != -1) {
      cout << "TPZAdaptMesh::CreateCompMesh mesh inconsistent\n";
    }
#endif

    /**PRefine precisar ser verificado!!!
       if(!csint) continue;
       //Refina em p o elemento criado
       //cmesh->SetName("depois criar elemento");
       //	cmesh->Print(cout);
       csint->PRefine(porders[el]);
       //	cmesh->SetName("depois prefine no elemento");
       //	cmesh->Print(cout);
       */
  }

  //Mais einh!!
  //	cmesh->SetName("Antes Adjust");
  //	cmesh->Print(cout);
  cmesh->AdjustBoundaryElements();
  //  cmesh->SetName("Depois");
  //  cmesh->Print(cout);

#ifdef HUGE_DEBUG
  if(check.CheckConnectOrderConsistency() != -1) {
    cout << "TPZAdaptMesh::CreateCompMesh mesh inconsistent\n";
  }
#endif

  return cmesh;
}

void TPZAdaptMesh::RemoveCloneBC(TPZCompMesh *mesh)
{
  int nelem = mesh->NElements();
  int iel;
  for(iel=0; iel<nelem; iel++) {
    TPZCompEl *cel = mesh->ElementVec()[iel];
    if(!cel) continue;
    int matid = cel->Material()->Id();
    if(matid == -1000) delete cel;
  }
}

void TPZAdaptMesh::DeleteElements(TPZCompMesh *mesh)
{
  int nelem = mesh->NElements();
  int iel;
  for(iel=0; iel<nelem; iel++) {
    TPZCompEl *cel = mesh->ElementVec()[iel];
    if(!cel) continue;
    TPZInterpolatedElement *cint = dynamic_cast<TPZInterpolatedElement *> (cel);
    if(!cint) continue;
    while(cint->HasDependency()) {
      TPZInterpolatedElement *large = LargeElement(cint);
      TPZInterpolatedElement *nextlarge = LargeElement(large);
      while(nextlarge != large) {
        large = nextlarge;
        nextlarge = LargeElement(large);
      }
      large->RemoveSideRestraintsII(TPZInterpolatedElement::EDelete);
      delete large;
    }
    cint->RemoveSideRestraintsII(TPZInterpolatedElement::EDelete);
    delete cint;
  }
}

TPZInterpolatedElement * TPZAdaptMesh::LargeElement(TPZInterpolatedElement *cint)
{
  int nc = cint->NConnects();
  int side;
  TPZInterpolatedElement *result = cint;
  for(side=0; side<nc; side++) {
    if(cint->Connect(side).HasDependency()) {
      TPZCompElSide cintside(cint,side);
      TPZCompElSide large = cintside.LowerLevelElementList(1);
      if(!large.Exists()) {
        cout << "TPZAdaptMesh::DeleteElements I dont understand\n";
        large = cintside.LowerLevelElementList(1);
        return cint;
      }
      result = dynamic_cast<TPZInterpolatedElement *> (large.Element());
      break;
    }
  }
  return result;
}

REAL TPZAdaptMesh::SortMinError(TPZVec<REAL> &ErrorVec,TPZVec<int> &perm, REAL percentage) {
  Sort(ErrorVec,perm);
  REAL error = 0.;
  //  REAL totalerror = 0.;
  //  REAL totaltruerror = 0.;
  //somatório dos componentes do vetor de erro
  int i,nelmesh;
  nelmesh = ErrorVec.NElements();
  for(i=0; i<nelmesh; i++) {
    error += ErrorVec[i];
    cout << ErrorVec[i] <<  "\t";
  }
  cout << "\nTPZAdaptMesh::GetAdaptMesh : Error = " << error << endl;

  REAL auxerror = 0.;
  REAL cutoffvalue = 0.;
  for(i=0;i<nelmesh;i++){
    auxerror += ErrorVec[perm[i]];
    if (auxerror >= percentage*error){
      cutoffvalue = ErrorVec[perm[i]];
      break;
    }
  }
  return cutoffvalue;
}

int TPZAdaptMesh::NPatches() {
  return fGeoRef.NElements();
}

TPZAdaptMesh::TPZCloneInfo::TPZCloneInfo() : fCloneMesh(0), fFineCloneMesh(0), fRefinedElements(), fRefinedOrders(),
					     fOriginalElements(), fOriginalOrders() {
}

TPZAdaptMesh::TPZCloneInfo::~TPZCloneInfo() {
  Cleanup();

  if(fFineCloneMesh) delete fFineCloneMesh;
  if(fCloneMesh) {
    TPZGeoMesh *gmesh = fCloneMesh->Reference();
    delete fCloneMesh;
    delete gmesh;
  }
}

void TPZAdaptMesh::TPZCloneInfo::AddSelectedElements(TPZVec<REAL> &ErrorVec, REAL minerror, TPZStack<TPZGeoEl *> &gelstack, TPZStack<int> &porders) {
  int norig = fOriginalElements.NElements();
  int orig;
  for(orig=0; orig<norig; orig++) {
    TPZCompEl *cel = fOriginalElements[orig]->Reference();
    if(!cel) {
      cout << "TPZCloneInfo::AddSelectedElements I dont understand1\n";
      continue;
    }
    int origindex = cel->Index();
    REAL error = ErrorVec[origindex];
    if(error < minerror) {
      gelstack.Push(fOriginalElements[orig]);
      porders.Push(fOriginalOrders[orig]);
    }
  }
  int nref = fRefinedElements.NElements();
  int iref;
  for(iref=0; iref<nref; iref++) {
    TPZGeoEl *gel = fRefinedElements[iref];
    TPZCompEl *cel = gel->Reference();
    if(cel) {
      int origindex = cel->Index();
      REAL error = ErrorVec[origindex];
      if(error >= minerror) {
        gelstack.Push(fRefinedElements[iref]);
        porders.Push(fRefinedOrders[iref]);
      }
      continue;
    }
    gel = gel->Father();
    if(!gel) {
      cout << "TPZCloneInfo::AddSelectedElements I dont understand 2\n";
      continue;
    }
    cel = gel->Reference();
    if(!cel) {
      cout << "TPZCloneInfo::AddSelectedElements I dont understand 3\n";
      continue;
    }
    int origindex = cel->Index();
    REAL error = ErrorVec[origindex];
    if(error >= minerror) {
      gelstack.Push(fRefinedElements[iref]);
      porders.Push(fRefinedOrders[iref]);
    }
  }
}


REAL TPZAdaptMesh::ComputeTrueError(TPZInterpolatedElement *coarse, void (*f)(TPZVec<REAL> &loc, TPZVec<REAL> &val, TPZFMatrix &deriv)){

  cout << "TPZAdaptMesh::ComputeTrueError needs to be implemented\n";
  return 0.;
}

void TPZAdaptMesh::TPZCloneInfo::PrintElementIds(ostream &out) {
/*  int norig = fOriginalElements.NElements();
  int orig;
  for(orig=0; orig<norig; orig++) {
    out << fOriginalElements[orig]->Id() << "/" << fOriginalOrders[orig] << " ";
  }
  out << "=== ";
  int nref = fRefinedElements.NElements();
  int iref;
  for(iref=0; iref<nref; iref++) {
    out << fRefinedElements[iref]->Id() << "/" << fRefinedOrders[iref] << " ";
  }*/
}
