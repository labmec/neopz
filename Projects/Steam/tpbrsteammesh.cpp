...
/// extract the state variables of the interface element
void TPBrSteamMesh::ExtractInletfaceState(TPZFMatrix &glob, TPZVec<REAL> &interfacestate)
{
	TPZManVector<int> equation,state;
...
//            if(statescales[state[jst]] != cellstatescale[jst])
//            {
//                std::stringstream sout;
//                sout << "jst " << jst << " glob scale " << statescales[state[jst]] << " loc scale " << cellstatescale[jst];
...
/// assemble the contribution of the cell
void TPBrSteamMesh::AssembleInterface(int interface, TPZFMatrix &ekinterface, TPZFMatrix &efinterface, TPZMatrix &glob, TPZFMatrix &res, TPZVec<REAL> &statescales)
{
	TPZManVector<int> equation,state;
    if(interface == 0)
    {
...
void TPBrSteamMesh::ComputeTangent(TPZMatrix &tangent, TPZFMatrix &residual, TPZVec<REAL> &statescales)
{
	tangent.Zero();
	residual.Zero();
...
/// compute the tangent matrix and residual
void TPBrSteamMesh::ComputeTangent2(TPZMatrix &tangent, TPZFMatrix &residual, TPZVec<REAL> &statescales)
{
	tangent.Zero();
	residual.Zero();
	TPZManVector<REAL,TPBrSteamFlux::NumInletVars> inlet(TPBrSteamFlux::NumInletVars,0);
...
REAL TPBrSteamMesh::LimitRange(TPZFMatrix &prevsol, TPZFMatrix &correction)
{
...
void TPBrSteamMesh::ProjectSolution(TPZFMatrix &solution)
{
    TPZManVector<REAL> facestate,cellstate,inletstate,facecorrection,cellcorrection,inletcorrection;
...
//        REAL temp = facestate[TPBrSteamFlux::EInletTemperature];
    ...
    for (int icell=0; icell<fNumCells; icell++) {
        ...
        {
//            REAL steamsat = cellstate[TPBrCellConservation::ESaturationSteam];
...
void TPBrSteamMesh::VerifyTangent(TPZFMatrix &direction)
{
    int neq = NumEquations();
	TPZFBMatrix bnd(neq,Bandwidth()),bnd2(neq,Bandwidth());
...
//    int ivar;
//    for (ivar=0; ivar<neq; ivar++) {
//        if(fabs(dirkeep(ivar,0)) < 1.e-6)
//        {
//            continue;
//        }
//        direction.Zero();
//        std::cout << "variable " << ivar << std::endl;
//        direction(ivar,0) = dirkeep(ivar,0);
        REAL alfa = 0.1;
        int count = 0;
