...
void TPZMultCamada::Solution(TPZVec < REAL > & Sol, TPZFMatrix & DSol, TPZFMatrix & axes, int var, TPZVec < REAL > & Solout) {
  ...
  if (var >= 50 && var <= 57) {
    ...
    for (i = 0; i < fCamadas.NElements(); i++) {
      if(var < 54) fCamadas[i]->Solution(Sol, DSol, axes, 50, SoloutAcc);
      else fCamadas[i]->Solution(Sol, DSol, axes, 54, SoloutAcc);
      for (j = 0; j < 3; j++) {
	Ma[j] += SoloutAcc[j];
	SoloutAcc[j] = 0.;
      }
...
void TPZMultCamada::Contribute(TPZVec<REAL> &x, TPZFMatrix &jac,TPZVec<REAL> &sol,TPZFMatrix &dsol,REAL weight,TPZFMatrix &axes,TPZFMatrix &phi,
			       TPZFMatrix &dphi, TPZFMatrix &ek, TPZFMatrix &ef) {

  int i;
  for (i = 0; i < fCamadas.NElements(); i++) fCamadas[i]->Contribute(x,jac,sol,dsol,weight,axes,phi,dphi,ek,ef);
}

void TPZMultCamada::ContributeBC(TPZVec < REAL > & x, TPZVec < REAL > & sol, REAL weight, TPZFMatrix & axes, TPZFMatrix & phi,
				 TPZFMatrix & ek, TPZFMatrix & ef, TPZBndCond & bc){
  if(fCamadas.NElements()) fCamadas[0]->ContributeBC(x,sol,weight,axes,phi,ek,ef,bc);
}
