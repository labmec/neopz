...
void TPZMatWithMemTranslator<TMEMTranslator, TFatherTranslator>::UpdateFromV3(TPZChunkInTranslation& chunk, const std::map<std::string, uint64_t>& toVersion) {
    ...
    chunk.mNewStream.Write(&size);
    for (i = 0; i < size; i++) {
        auto objId = TPZPersistenceManager::NewChunkInTranlation();
        TPZAutoPointer<TPZChunkInTranslation> fMemoryChunk = new TPZChunkInTranslation(objId, tMEMTranslator.GetClassId(), chunk.mOldStream, chunk.mOldVersion);
        TPZPersistenceManager::SetChunk(objId, fMemoryChunk);
        tMEMTranslator.UpdateStream(*fMemoryChunk, toVersion); //fMemory[i]
        chunk.mOldStream = fMemoryChunk->mOldStream;
        fMemoryChunk->mOldStream.clear();
