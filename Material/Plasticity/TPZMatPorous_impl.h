...
template <class T, class TMEM>
void TPZMatPorous<T, TMEM >::Print(std::ostream &out, const int memory) const
{
    out << this->Name();
...
template <class T, class TMEM>
int TPZMatPorous<T, TMEM >::VariableIndex(const std::string &name) const
{
...
template <class T, class TMEM>
int TPZMatPorous<T, TMEM >::NSolutionVariables(int var) const
{
...
template <class T, class TMEM>
void TPZMatPorous<T, TMEM >::Solution(const TPZMaterialDataT<STATE> &data, int var, TPZVec<REAL> &Solout)
{
...
template <class T, class TMEM>
void TPZMatPorous<T, TMEM >::Contribute(const TPZMaterialDataT<STATE> &data,
                                        REAL weight,
                                        TPZFMatrix<REAL> &ek, TPZFMatrix<REAL> &ef)
{
    ...
    if(this->fForcingFunction)
        this->fForcingFunction(x,this->m_force);
...
template <class T, class TMEM>
void TPZMatPorous<T, TMEM >::ContributeBC(const TPZMaterialDataT<STATE> &data,
                                          REAL weight,
                                          TPZFMatrix<REAL> &ek,
                                          TPZFMatrix<REAL> &ef,
...
{
    TPZMatSingleSpaceT<STATE>::Contribute(data, weight, ef);//not efficient but here to remember reimplementing it when Contribute becomes robust
}
...
void TPZMatPorous<T, TMEM >::ContributeBC(const TPZMaterialDataT<STATE> &data,
                                          ...
                                          TPZBndCondT<STATE> &bc)
{
    TPZMatSingleSpaceT<STATE>::ContributeBC(data, weight, ef, bc);//not efficient but here to remember reimplementing it when ContributeBC becomes robust
}
...
template <class T, class TMEM>
TPZMaterial * TPZMatPorous<T, TMEM >::NewMaterial() const 
{
    return new TPZMatPorous<T, TMEM>(*this);
}

template <class T, class TMEM>
std::string TPZMatPorous<T, TMEM >::Name() const
{
...
template <class T, class TMEM>
void TPZMatPorous<T, TMEM >::FillDataRequirements(TPZMaterialData &data) const
{
...
template <class T, class TMEM>
void TPZMatPorous<T, TMEM >::ComputePorePressure(const TPZMaterialDataT<STATE> & data, REAL & Pp, TPZVec<REAL> & dPp)
{
    int dim = Dimension(), i;
...
template <class T, class TMEM>
void TPZMatPorous<T, TMEM >::UpdatePorePressure(const TPZMaterialDataT<STATE> & data)
{
    int dim = Dimension(), i;
    int intPt = data.intGlobPtIndex;
