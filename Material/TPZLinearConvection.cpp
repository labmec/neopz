...
void TPZLinearConvection::Contribute(TPZVec<REAL> &x,TPZVec<REAL> &sol,TPZFMatrix &dsol, REAL weight,
    ...
    int i;
    for(i=0; i<3; i++) daxesdksi(i,i) = 1.;
    Contribute(x,daxesdksi,sol,dsol,weight,axes,phi,dphi,ek,ef);
}
...
void TPZLinearConvection::Contribute(TPZVec<REAL> &x,TPZFMatrix &daxesdksi,TPZVec<REAL> &sol,TPZFMatrix &dsol, REAL weight,
  			  TPZFMatrix &axes,TPZFMatrix &phi,TPZFMatrix &dphi,TPZFMatrix &ek,TPZFMatrix &ef) {
    REAL convectax[2];
