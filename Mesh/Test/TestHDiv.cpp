...
TPZAutoPointer<TPZCompMesh> GenerateMesh(int type, int order);
...
{
    int eltype;
    for (eltype = 0; eltype < 2; eltype++) {
        TPZAutoPointer<TPZCompMesh> cmesh = GenerateMesh(eltype,3);
        TPZGeoMesh *gmesh = cmesh->Reference();
        int ncorner = (eltype == 0) ? 4 : 3;
        TPZManVector<int,4> nodeids(ncorner,0), nodesperm(ncorner,0);
        int elindex = (eltype == 0) ? 4 : 8;
        TPZGeoEl *gel = gmesh->ElementVec()[elindex];
...
static void CheckDRham(TPZInterpolatedElement *intel)
{
    TPZFNMatrix<120> inner, multiplier;
    TPZAutoPointer<TPZMatrix> L2 = new TPZFNMatrix<36>;
...
//    std::cout << "shape indices " <<  dataA.fVecShapeIndex << std::endl;
//    TPZFNMatrix<200> div(np,2+nflux);
    for (ip=0; ip<np; ip++) {
        REAL weight;
...
//        div(ip,1) = pos[1];
        ...
        //dataA.axes.Print("axes");
        //dataA.dphix.Print("dphix");
        int ish,jsh;
        for (ish=firstpressure; ish<lastpressure; ish++) {
            ...
            for (jsh=0; jsh<nflux; jsh++) {
                ...
                //std::cout << "jsh " << jsh << " vecinner " << vecinner << " divphi " << divphi << std::endl;
//                div(ip,2+jsh) = divphi;
                inner(ish-firstpressure,jsh) += dataA.phi(ish,0)*divphi*weight*fabs(dataA.detjac);
            }
        }
    }
//    std::ofstream divfile("divergence.nb");
//    div.Print("divergence = ",divfile,EMathematicaInput);
}
