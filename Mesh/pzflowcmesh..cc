...
REAL TPZFlowCompMesh::MaxVelocityOfMesh(){
  ...
  for(i=0;i<nel;i++){
    ...
    if(!pElComp)PZErr << "TPZFlowCompMesh::MaxVelocityOfMesh: No associated computation element\n".;

    TPZMaterial *mat = pElComp->Material();
    TPZGeoEl *pElGeo = pElComp->Reference();
    if(!mat || !pElGeo)PZErr << "TPZFlowCompMesh::MaxVelocityOfMesh ERROR: null material or element.\n";

    dim = mat->Dimension();
    elDim = pElGeo->Dimension();
    if(elDim != dim)PZErr << "TPZFlowCompMesh::MaxVelocityOfMesh2 ERROR: element and material with different dimensions.\n";

    TPZConservationLaw2 *law = dynamic_cast<TPZConservationLaw2 *>(mat);
    if(!law)PZErr << "TPZFlowCompMesh::MaxVelocityOfMesh2 ERROR: non-fluid material.\n";
    ...
    if(density[0] < 0.0)PZErr << "TPZFlowCompMesh::MaxVelocityOfMesh: Negative density\n";
    ...
    if(press < 0.0)PZerr << "TPZFlowCompMesh::MaxVelocityOfMesh minus pressure\n";
...
{
   int i, NumMat;
   TPZAdmChunkVector< TPZMaterial * > * pVecMat;

   pVecMat = fCompMesh->MaterialVec();
   NumMat = pVecMat->NElements();

   // buffering the fluid materials
   for(i = 0; i < NumMat; i++)
   {
      TPZConservationLaw2 * pConsLaw;
      pConsLaw = dynamic_cast<TPZConservationLaw2 *>(pVecMat->operator[](i));
      if(pConsLaw)
      {
