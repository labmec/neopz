...
//    }
    ...
    if(store_errors && this->Mesh()->ElementSolution().Cols() < NErrors)
    {
        auto nrows = Mesh()->NElements();
        this->Mesh()->ElementSolution().Resize(nrows,NErrors);
    }
    ...
    for(int nint = 0; nint < nintpoints; nint++) {
        ...
        if(fp) {
            fp(datavec[iactive].x,u_exact,du_exact);
        }
