//$Id: pzinterpolationspace.cpp,v 1.35 2009-06-17 22:02:15 fortiago Exp $
...
/*void TPZInterpolationSpace::ComputeNormalVec(TPZMaterialData &data){
	
#ifdef LOG4CXX 
	{
		std::stringstream sout;
		sout << endl<<"Log do metodo ComputeNormalVec"<< std::endl;
		LOGPZ_DEBUG(logger,sout.str())
	}
#endif	
	int v=0; //Contador dos vetores
	int ind=0; //Contador de index ou funções de forma em Hdiv
	data.VecIndex.Resize(0);
	data.ShapeIndex.Resize(0);
	//                                 2
	data.fNormalVec.Resize(this->Dimension()*this->Reference()->NSides(),3);
	
	//cout << "tamanho do fnormalVec---"<<this->Dimension()*this->Reference()->NSides()<<endl;
	...
	for(int isideEl=0; isideEl< nSidelEl; isideEl++)// loop sobre os lados do elemento
	{
		...
		if(this->Reference()->SideDimension(isideEl)== this->Dimension()-1) // se lado i tem dimensão n-1
		{
#ifdef	LOG4CXX
			{
				std::stringstream sout;
...
#endif
			for(int jnode=0;jnode<this->Reference()->NSideNodes(isideEl);jnode++) //loop sob os nós do lado
			{
#ifdef LOG4CXX
				{
					std::stringstream sout;
...
#endif
				TPZStack<TPZGeoElSide,100> elsides;
							
				
				int sideNod  = this->Reference()->SideNodeLocIndex(isideEl,jnode);
				this->Reference()->AllHigherDimensionSides(sideNod,1, elsides);// calcula todos os vizinhos de dimensão 1 do noh
#ifdef LOG4CXX
				{
					std::stringstream sout;
					sout << "		loop sobre os vizinhos 1D do no "<< jnode<< endl;
					LOGPZ_DEBUG(logger,sout.str())
				}
#endif
				
				for(int l=0;l<elsides.NElements();l++)
				{
#ifdef LOG4CXX
					{
						std::stringstream sout;
						sout << "		vizinho: "<< elsides[l].Side() << endl;
						LOGPZ_DEBUG(logger,sout.str())
					}
#endif
					if(elsides[l].Side()!=isideEl)// se o vizinho é diferente do próprio lado que contem o nó
					{
#ifdef LOG4CXX
						{
							std::stringstream sout;
							sout << "		este eh o vizinho q eh diferente do proprio lado que contem o noh " << endl;
							sout << "		loop sobre os nohs do lado vizinho " << endl;

							LOGPZ_DEBUG(logger,sout.str())
						}
#endif
						for(int k=0;k< elsides[l].NSideNodes();k++)// loop sobre os nós do lado vizinho
						{
							
							
							if(elsides[l].SideNodeIndex(k)!=this->Reference()->SideNodePtr(isideEl,jnode)->Id())// se o nó do vizinho é diferente nó de referencia
							{
#ifdef LOG4CXX
								{
									std::stringstream sout;
								sout <<"				no referencia-> "<< this->Reference()->SideNodePtr(isideEl,jnode)->Id()<< endl;
								sout <<"				no complementar->"<< elsides[l].SideNodeIndex(k)<< endl;
								
								sout<< "				ref: "<<this->Reference()->SideNodePtr(isideEl,jnode)->Id()<< endl;
								sout<< "				comp: "<<elsides[l].SideNodeIndex(k)<< endl;
								
								sout <<"				Coord x referencia:"<< this->Reference()->SideNodePtr(isideEl,jnode)->Coord(0)<< endl;
								sout <<"				Coord y referencia:"<< this->Reference()->SideNodePtr(isideEl,jnode)->Coord(1)<< endl;
								sout <<"				Coord x complementar:"<< this->Reference()->NodePtr(elsides[l].SideNodeLocIndex(k))->Coord(0)<< endl;
								sout <<"				Coord y complementar:"<< this->Reference()->NodePtr(elsides[l].SideNodeLocIndex(k))->Coord(1)<< endl;
									LOGPZ_DEBUG(logger,sout.str())
								}
#endif			

								//construcao dos vetores
								data.fNormalVec(v,0)=(this->Reference()->SideNodePtr(isideEl,jnode)->Coord(0)) - this->Reference()->NodePtr(elsides[l].SideNodeLocIndex(k))->Coord(0);
								data.fNormalVec(v,1)= (this->Reference()->SideNodePtr(isideEl,jnode)->Coord(1)) - this->Reference()->NodePtr(elsides[l].SideNodeLocIndex(k))->Coord(1);
								data.fNormalVec(v,2)= 0.;

#ifdef LOG4CXX
								{
									std::stringstream sout;
									sout << endl;
								//sout<< "			lado="<<this->Reference()->SideNodePtr(isideEl,jnode)->Id()<< endl;
								sout<< "			lado="<<sideNod<< endl;
								sout<< "			numFuncoes="<<NConnectShapeF(sideNod)<<endl;
								//sout<< "			numFuncoes="<<NConnectShapeF(this->Reference()->SideNodePtr(isideEl,jnode)->Id())<<endl;
								//sout<< "			firsIndex="<<FirstShapeIndex(this->Reference()->SideNodePtr(isideEl,jnode)->Id())<< endl<< endl<<endl;
									sout<< "			firsIndex="<<FirstShapeIndex(sideNod)<< endl<< endl<<endl;
									LOGPZ_DEBUG(logger,sout.str())
								}
#endif
								int tamanho1=data.VecIndex.NElements()+NConnectShapeF(sideNod);
								//data.VecIndex.Resize(data.VecIndex.NElements()+NConnectShapeF(this->Reference()->SideNodePtr(isideEl,jnode)->Id()));
								//data.ShapeIndex.Resize(data.ShapeIndex.NElements()+NConnectShapeF(this->Reference()->SideNodePtr(isideEl,jnode)->Id()));
								data.VecIndex.Resize(tamanho1);
								data.ShapeIndex.Resize(tamanho1);
								for(int w=0; w < NConnectShapeF(sideNod);w++)
								{
									data.VecIndex[ind] = v ; 
									data.ShapeIndex[ind] = FirstShapeIndex(sideNod)+w;//this->Reference()->SideNodePtr(isideEl,jnode)->Id())+ w ;
									ind++;
								}
								v++;
							}
			...
			//aqui (neste nível) construir vetores normais
			
#ifdef LOG4CXX
			{
				std::stringstream sout;
				sout << "construindo os vetores normais das arestas"<<endl;
				LOGPZ_DEBUG(logger,sout.str())
			}
#endif
			data.fNormalVec(v,0)=    this->Reference()->SideNodePtr(isideEl,1)->Coord(1) - this->Reference()->SideNodePtr(isideEl,0)->Coord(1);
			data.fNormalVec(v,1)= -( this->Reference()->SideNodePtr(isideEl,1)->Coord(0) - this->Reference()->SideNodePtr(isideEl,0)->Coord(0));
			data.fNormalVec(v,2)=0.;
			
			REAL norma= sqrt(data.fNormalVec(v,0)*data.fNormalVec(v,0)+data.fNormalVec(v,1)*data.fNormalVec(v,1)+data.fNormalVec(v,2)*data.fNormalVec(v,2));
			data.fNormalVec(v,0) *=1./norma;
			data.fNormalVec(v,1) *=1./norma;
			data.fNormalVec(v,2) *=1./norma;
			// fator que "unitariza" os vetores
			REAL tempA= data.fNormalVec(v,0)*data.fNormalVec(v-1,0)+data.fNormalVec(v,1)*data.fNormalVec(v-1,1)+data.fNormalVec(v,2)*data.fNormalVec(v-1,2);
			data.fNormalVec(v-1,0) *=1./tempA;
			data.fNormalVec(v-1,1) *=1./tempA;
			data.fNormalVec(v-1,2) *=1./tempA;
			
			REAL tempB= data.fNormalVec(v,0)*data.fNormalVec(v-2,0)+data.fNormalVec(v,1)*data.fNormalVec(v-2,1)+data.fNormalVec(v,2)*data.fNormalVec(v-2,2);
			data.fNormalVec(v-2,0) *=1./tempB;
			data.fNormalVec(v-2,1) *=1./tempB;
			data.fNormalVec(v-2,2) *=1./tempB;
#ifdef LOG4CXX
			{
				std::stringstream sout;
				sout << endl;
		...
		if(this->Reference()->SideDimension(isideEl)== this->Dimension())
		{
#ifdef LOG4CXX
			{
				std::stringstream sout;
			...
			for(int m=0;m<smallsides.NElements();m++)
			{
				...
				{
					data.fNormalVec(v,0)=(this->Reference()->SideNodePtr(smallsides[m],1)->Coord(0))-(this->Reference()->SideNodePtr(smallsides[m],0)->Coord(0));				
					data.fNormalVec(v,1)=(this->Reference()->SideNodePtr(smallsides[m],1)->Coord(1))-(this->Reference()->SideNodePtr(smallsides[m],0)->Coord(1));
					REAL norma= sqrt(data.fNormalVec(v,0)*data.fNormalVec(v,0)+data.fNormalVec(v,1)*data.fNormalVec(v,1)+data.fNormalVec(v,2)*data.fNormalVec(v,2));
					data.fNormalVec(v,0) *=1./norma;
					data.fNormalVec(v,1) *=1./norma;
					data.fNormalVec(v,2) *=1./norma;
...
#endif
			TPZVec<REAL> meio(3,0.), mestre(3,0.);
			this->Reference()->X(mestre,meio);
			
			
			this->Reference()->Jacobian( meio, data.jacobian, data.axes, data.detjac , data.jacinv);
#ifdef LOG4CXX
			{
				std::stringstream sout;
			data.axes.Print( "axes ",sout);
			sout << endl<< "				meio: "<<"("<< meio[0] << ","<< meio[1]<<","<< meio[2]<<")"<< endl;
				LOGPZ_DEBUG(logger,sout.str())
			}
#endif
			data.fNormalVec(v,0)=data.axes.GetVal(0,0);
			data.fNormalVec(v,1)=data.axes.GetVal(1,0);
#ifdef LOG4CXX
			{
				std::stringstream sout;
			...
			v++;
			data.fNormalVec(v,0)=data.axes.GetVal(0,1);
			data.fNormalVec(v,1)=data.axes.GetVal(1,1);
			cout<<"tamanho do Vec Index = "<<data.VecIndex.NElements()<<endl;
...
/*void TPZInterpolationSpace::ComputeHdivIndex(TPZMaterialData &data){
	std::stringstream sout;
#ifdef LOG4CXX 
	{		
		std::stringstream sout;
		sout << endl<<"Log do metodo ComputeHdivIndex"<< endl;
		LOGPZ_DEBUG(logger,sout.str())
	}
#endif	
	int v=0; //Contador dos vetores
