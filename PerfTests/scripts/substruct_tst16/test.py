...
def setup_cmd():
	...
	# Check run directory
	rundir = os.path.join(builddir,'scripts','substruct_tst16')
	...
	# Check micnativelodex to run application on Xeon Phi
	micloadex = "opt/intel/mic/bin/micnativeloadex"
	if not os.path.isfile(micloadex) :
	error(micloadex+'is an invalid path to the mic load tool.', 1)
	# Copy input file to the Xeon Phi
        os.system ("scp "+inputfn+" mic0:/home/edson")
        # Put the arguments together
        arguments = ' -mp '+ "/home/edson/8andares02.txt"
	arguments = arguments + ' -nsub 240'
	arguments = arguments + ' -nt_a 240' 
	arguments = arguments + ' -nt_d 240' 
	arguments = arguments + ' -nt_m 240' 
	arguments = arguments + ' -nt_sm 240' 
	...
	return rundir, micloadex + ' ' + executable + ' -a ' + '\"' + arguments + '\"'
...
def short_description() : return "substructure -- 8andares02.txt -- 240 substructures and 240 threads"
