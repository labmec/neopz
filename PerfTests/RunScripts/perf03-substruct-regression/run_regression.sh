...
then 
	...
	cmake -DREAL_TYPE=double -DCMAKE_BUILD_TYPE=Release -DBUILD_PERF_TESTS=ON -DBUILD_PROJECTS=OFF -DBUILD_TUTORIAL=OFF -DBUILD_UNITTESTING=OFF \
		-DUSING_METIS=ON -DUSING_LOG4CXX=OFF -DUSING_FAD=OFF -DUSING_BOOST=ON -DUSING_OPENSSL=ON -DUSING_TBB=ON \
			-DPERFTEST_DATA_DIR:PATH=$DIR_BASE/checkpoints -DPERFTEST_APPS_DIR:PATH=$DIR_BASE/work/neopz_trunk_build/PerfTests \
