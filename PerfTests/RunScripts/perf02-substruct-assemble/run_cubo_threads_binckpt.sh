...
function run_cfg
{
    ...
    for ns in 1 2 4 8; do
        ...
        BASEOUT="cubo1.@REAL_TYPE@.bin.ckpt1.p$P.nsub$ns.nt_a.$NTA.nt_d.$NTD.nt_m.$NTM.nt_sm.$NTSM"

        IF="cubo1.p$P.nsub$ns.t.@REAL_TYPE@.bin.ckpt1"
        OF="cubo1.p$P.nsub$ns.t.@REAL_TYPE@.bin.ckpt3"
        CMD="$APP -bc -cf1 @PERFTEST_LARGE_DATA_DIR@/SubStruct/inputs/$IF -dc3 $OF -st3 -ass_rdt $BASEOUT.ass.rdt -cre_rdt $BASEOUT.cre.rdt " 
