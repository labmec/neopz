#ifndef _tinyfadfifteen_h_
#define _tinyfadfifteen_h_
...
template <class T> class TinyFad<15,T> {
public:
...
public:
  ...
  TinyFad(const TinyFad<15,T> & in) : n(0), val_(in.val_) {
     dx0_ = in.dx0_;
     dx1_ = in.dx1_;
     dx2_ = in.dx2_;
     dx3_ = in.dx3_;
  ...
  TinyFad<15,T> & operator = (const T & in){
     val_ = in;
  ...
  TinyFad<15,T> & operator += (const T & in){     val_ += in;
...
template <class T> inline TinyFad<15,T> operator + (const TinyFad<15,T>& in)
{
  return TinyFad<15,T>(in);
}
...
TinyFad<15,typename NumericalTraits<L,R>::promote >
operator +(const TinyFad<15,L>& un, const TinyFad<15,R>& deux) {

  typedef typename NumericalTraits<L,R>::promote value_type;

  No_Initialization nothing;
  TinyFad<15,value_type> tmp( nothing );
...
template <class L, class R> inline
TinyFad<15,typename NumericalTraits<L,R>::promote>
operator +(const TinyFad<15,L>& un, const R& deux) {
...
template <class L, class R> inline
TinyFad<15,typename NumericalTraits<L,R>::promote >
operator +(const L& un, const TinyFad<15,R>& deux) {
...
operator /(const TinyFad<15,L> & un, const TinyFad<15,R> & deux) {
  ...
  if (deux.val() == 0.) error("TinyFad & TinyFad::operator /(const TinyFad<15,L> & un, const TinyFad<15,R> & deux), dividing by 0");
...
operator /(const L & un, const TinyFad<15,R> & deux) {
  ...
  if (deux.val() == 0.) error("TinyFad & TinyFad::operator /(const L & un, const TinyFad<15,R> & deux), dividing by 0");
...
operator /(const TinyFad<15,L> & un, const R & deux) {
  ...
  if (deux == 0.) error("TinyFad & TinyFad::operator /(const TinyFad<15,L> & un, const R & deux), dividing by 0");
...
template <class T> TinyFad<15,T> exp (const TinyFad<15,T>& in)
{
  TinyFad<15,T> tmp(exp(in.val()));
...
template <class T> TinyFad<15,T> log (const TinyFad<15,T>& in)
{
  if ( in.val() <= 0) error("TinyFad log (const TinyFad& in) : zero or negative value");
  TinyFad<15,T> tmp(log(in.val()));
...
template <class T> TinyFad<15,T> sqrt (const TinyFad<15,T>& in)
{
  if ( in.val() < 0. ) error("TinyFad<15,T> sqrt (const TinyFad& in) : negative value");
  TinyFad<15,T> tmp(sqrt(in.val()));
  ...
  if ( in.val() == T(0.) ){
    ...
    if ( !test )
      error("TinyFad<15,T> sqrt (const TinyFad& in) : null value");
  }
  else {
...
template <class T> TinyFad<15,T> sin (const TinyFad<15,T>& in)
{
  TinyFad<15,T> tmp( sin(in.val()) );
...
template <class T> TinyFad<15,T> cos (const TinyFad<15,T>& in)
{
  TinyFad<15,T> tmp(cos(in.val()));
...
template <class T> TinyFad<15,T> tan (const TinyFad<15,T>& in)
{
  if ( in.val() == 0) error("TinyFad tan (const TinyFad& in) undiefined in 0.");
  TinyFad<15,T> tmp(tan(in.val()));
...
template <class T> TinyFad<15,T> pow (const TinyFad<15,T>& in, double e)
{
  TinyFad<15,T> tmp(pow(in.val(), e));
...
template <class T> TinyFad<15,T> pow (const TinyFad<15,T>& un, const TinyFad<15,T>& deux)
{
  if (un.val() == 0) error("TinyFad pow (const TinyFad& un, const TinyFad& deux), un = 0. ");
  TinyFad<15,T> tmp(pow(un.val(), deux.val()));
...
template <class T> TinyFad<15,T> pow (const TinyFad<15,T>& in, const int e)
{
  TinyFad<15,T> tmp( pow((double)in.val(), (double)e) );
  ...
  tmp.d6() = e*in.d6()*pow((double)in.val(), (double)e-1);
  tmp.d7() = e*in.d7()*pow((double)in.val(), (double)e-1);
  tmp.d8() = e*in.d8()*pow((double)in.val(), (double)e-1);
  tmp.d9() = e*in.d9()*pow((double)in.val(), (double)e-1);
  tmp.d10() = e*in.d10()*pow((double)in.val(), (double)e-1);
  tmp.d11() = e*in.d11()*pow((double)in.val(), (double)e-1);
  tmp.d12() = e*in.d12()*pow((double)in.val(), (double)e-1);
  tmp.d13() = e*in.d13()*pow((double)in.val(), (double)e-1);
  tmp.d14() = e*in.d14()*pow((double)in.val(), (double)e-1);

  return tmp;
}

template <class T> TinyFad<15,T> abs (const TinyFad<15,T>& in)
{
...
template <class T> std::ostream& operator << (std::ostream& os, const TinyFad<15,T>& a)
{
