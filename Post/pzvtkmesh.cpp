...
void TPZVTKGraphMesh::DrawSolution(int step, REAL time){
	...
	int n;
	if(fScalarNames.NElements())
	{
		{
			std::stringstream sout;
			sout << fFileName.substr(0,fFileName.size()-4) << ".scal_" << step << ".vtk";
			fOutFile.open(sout.str().c_str());
		}
		(fOutFile) << "# vtk DataFile Version 3.0" << endl;
		(fOutFile) << "Arquivo gerado por PZ" << endl;
		(fOutFile) << "ASCII\n" << endl;
		(fOutFile) << "DATASET POLYDATA" << endl;
		DrawNodes();
		DrawConnectivity();
		...
		(fOutFile) << "POINT_DATA " << NPoints() << endl;
		for(n=0; n<numscal; n++)
		{
	...
	if(fVecNames.NElements())
	{
		...
		{
			std::stringstream sout;
			sout << fFileName.substr(0,fFileName.size()-4) << ".vec_" << step << ".vtk";
			fOutFile.open(sout.str().c_str());
		}
		(fOutFile) << "# vtk DataFile Version 3.0" << endl;
		(fOutFile) << "Arquivo gerado por PZ" << endl;
		...
		for(n=0; n<numvec; n++)
		{
			(fOutFile) << "VECTORS " << fVecNames[n] << " float" << endl << "LOOKUP_TABLE default\n";
			int nnod = fNodeMap.NElements(), i;
			for(i=0;i<nnod;i++) {
...
void TPZVTKGraphMesh::DrawConnectivity() {
	...
	for(i=0;i<nel;i++) {
		...
		if(el) nelem += el->NElements();
 	}
	(fOutFile) << nelem << " " << nelem*5 << std::endl;
	for(i=0;i<nel;i++) {
		el = (TPZGraphEl *) fElementList[i];
